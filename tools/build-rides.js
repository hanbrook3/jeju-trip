...
(async () => {
  ...
  /* ── 좌표 없는 정차지를 사이에 둔 구간 ──────────────────────────────
     위 고리는 이웃끼리만 훑으므로 `군산오름 → [하산] → 저녁` 같은 구간이 통째로
     안 재진다. 그런데 그 주행은 실제로 있고, 배포판 시각 차이가 체류로 흡수하고
     있을 뿐이다. **RIDE 에 담으면 안 된다** — 앱의 dayPlan 도 배포판 정차지는
     이어 붙이지 않으므로, 담으면 같은 주행을 체류와 이동으로 두 번 세게 된다.
     여기서는 담지 않고 **일정 안에 들어가는지만** 본다. 안 재면 "실측이 다 들어간다"
     는 말이 거짓이 된다 — 실제로 2일차 그 구간은 59분인데 자리가 60분뿐이었다. */
  const 사이 = [];
  for (let d = 0; d < D.length; d++) {
    const s = D[d].stops;
    for (let i = 0; i < s.length; i++) {
      if (길좌표(s[i])) continue;
      let 앞 = i - 1; while (앞 >= 0 && !길좌표(s[앞])) 앞--;
      let 뒤 = i + 1; while (뒤 < s.length && !길좌표(s[뒤])) 뒤++;
      if (앞 < 0 || 뒤 >= s.length) continue;          /* 하루의 한쪽 끝이면 잴 것이 없다 */
      const 이시각 = 분(s[i].t), 뒤시각 = 분(s[뒤].t);
      if (이시각 === null || 뒤시각 === null) continue;
      const r = await 재기(길좌표(s[앞]), 길좌표(s[뒤]), 떠나는때(d, 이시각));
      if (r.오류) { 사이.push(`${d + 1}일차 ${s[앞].n} → [${s[i].n}] → ${s[뒤].n} : ${r.오류}`); continue; }
      const 자리 = 뒤시각 - 이시각;
      사이.push(`${d + 1}일차 ${s[앞].n} → [${s[i].n}] → ${s[뒤].n} : 실측 ${r.분}분 ${r.km}km · 자리 ${자리}분`
        + (자리 - r.분 >= 0 ? ` (여유 ${자리 - r.분}분)` : ` ⚠ ${r.분 - 자리}분 모자람`));
    }
  }
  ...
  if (사이.length) {
    console.log('\n※ 좌표 없는 정차지를 사이에 둔 구간 ' + 사이.length + '개 — RIDE 에 담지 않는다'
      + '(그 주행은 배포판 체류에 이미 들어 있다). 자리에 들어가는지만 본다:');
    사이.forEach(x => console.log('   ' + x));
  }
