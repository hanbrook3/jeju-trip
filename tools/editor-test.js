...
/* **「상시 개방」 이라 적힌 곳의 괄호 속 시간은 폐장 시각이 아니다** — 딸린 시설
   (내부 관람·배 체험·숍) 것이라 그것으로 문을 닫는다고 하면 헛경고가 뜬다. */
확인('상시 개방이면 괄호 속 시각을 안 잡는다',
  API.openHours('외부 상시 개방 · 내부 관람은 일정에 따라 제한 (보통 10:00~16:00)'), null);
확인('계곡 상시 개방도 마찬가지', API.openHours('계곡 상시 개방 · 배 체험 09:00 ~ 18:00'), null);
확인('상시 개방은 시각 없음', API.openHours('상시 개방'), null);
확인('괄호 안 시각은 상시 개방이 아니면 잡는다', API.openHours('관람 (보통 10:00~16:00)'), {open:600,close:960});
/* 맛집 hours 는 붙임표가 en dash 다 */
확인('en dash 도 읽는다', API.openHours('09:30–20:00 · 목 휴무'), {open:570,close:1200});
...
/* 맛집은 off 칸이 없고 hours 문구 안에 휴무가 뭉쳐 있다 */
확인('한 글자 요일 + 휴무', API.offDay('09:30–20:00 · 브레이크 15:30–17:00 · 목 휴무'), '목');
확인('요일 + 정기 휴무', API.offDay('월요일 정기 휴무'), '월');
확인('매주 + 요일 + 휴무', API.offDay('매주 목요일 휴무 · 영업시간 확인 필요'), '목');
확인('매달 격주는 안 잡는다', API.offDay('08:00–20:00 · 매달 2·4번째 수요일 휴무'), null);
확인('정기휴무 없음은 안 잡는다', API.offDay('16:00–22:00 · 정기휴무 없음'), null);
확인('휴무일 없음도 안 잡는다', API.offDay('10:30–20:00 · 정해진 휴무일 없음 — 가기 전 전화 권장'), null);
...
console.log('\n[미리보기 문구의 개수]');
/* 카톡·메신저 미리보기(og:description, meta description)는 화면에 안 보여서
   여행지·맛집을 늘려도 조용히 낡는다. 배열 길이와 대 본다. */
(function () {
  const 배열 = function (이름) {
    const i = h.indexOf('const ' + 이름 + '=[');
    const j = h.indexOf('\n];', i);
    return eval(h.slice(i + ('const ' + 이름 + '=').length, j + 2));
  };
  const 여행지수 = 배열('SPOT').length, 맛집수 = 배열('FOOD').length;
  const 나온것 = [];
  const re = /content="[^"]*여행지\s*(\d+)\s*곳[^"]*맛집\s*(\d+)\s*곳[^"]*"/g;
  let m;
  while ((m = re.exec(h))) 나온것.push([+m[1], +m[2]]);
  확인('미리보기 문구가 두 군데 있다', 나온것.length, 2);
  나온것.forEach(function (x, n) {
    확인('미리보기 ' + (n + 1) + ' 의 여행지 수', x[0], 여행지수);
    확인('미리보기 ' + (n + 1) + ' 의 맛집 수', x[1], 맛집수);
  });
})();
...
(function () {
  ...
  확인('한 자리에 핀이 둘인 곳', 겹친자리, [
    '2일차 숙소 출발↔숙소 도착',
    '3일차 섭지코지 주차장↔섭지코지 주차장 복귀',
    '3일차 성산일출봉 주차장↔성산일출봉 주차장 복귀',
    '4일차 숙소 출발↔숙소 도착 · 짐 싸고 쉬기', '5일차 체크아웃↔숙소 출발']);
